...
File Purpose: To create a skeleton autonomous mode starting from position two
"""
...
class autonomousModeFromSecondPosition(StatefulAutonomous):

    MODE_NAME = 'Second Position Auto'
    DEFAULT = False
...
# initially stopping the bot using a timed state
    ...
    def drive_forward_towards_defense(self) :
        if (self.drive.lfencoder.getDistance() + self.drive.rfencoder.getDistance()) <= 75 :
            self.drive.autonTankDrive(0.4, 0.4)
        else :
