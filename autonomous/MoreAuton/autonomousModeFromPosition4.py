...
File Purpose: To create a skeleton autonomous mode starting from position four
"""
...
class autonomousModeFromFourthPosition(StatefulAutonomous):

    MODE_NAME = 'Fourth Position Auto'
    DEFAULT = False
...
# initially stopping the bot using a timed state
    ...
    def rotate_to_the_left(self):
        self.drive.turn_angle(78)
        self.next_state('drive_forward_towards_defense')

    @state()
    def drive_forward_towards_defense(self) :
        if (self.drive.lfencoder.getDistance() + self.drive.rfencoder.getDistance()) <= 154 :
            self.drive.autonTankDrive(0.4, 0.4)
        else :
