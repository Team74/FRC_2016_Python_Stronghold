...
File Purpose: To create a skeleton autonomous mode starting from position three
"""
...
class autonomousModeFromThirdPosition(StatefulAutonomous):

    MODE_NAME = 'Third Position Auto'
    DEFAULT = False
...
# initially stopping the bot using a timed state
    ...
    def rotate_to_the_left(self):
        self.drive.turn_angle(70)
        self.next_state('drive_forward_towards_defense')

    @state()
    def drive_forward_towards_defense(self) :
        if (self.drive.lfencoder.getDistance() + self.drive.rfencoder.getDistance()) <= 114 :
            self.drive.autonTankDrive(0.4, 0.4)
        else :
